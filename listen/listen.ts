...
type Onset = { t: number; midi: number; hz: number; cents: number; rms: number }
type Now = { hz: number; midi: number; cents: number; rms: number } | null
...
function frame() {
  ...
  if (attack) { armedAt = t; stable = 0 } // count settled frames from the attack, not before it
  if (rms < GATE) {
    state.now = null
    silentFrames++
    stable = 0
  ...
  state.now = { hz: Math.round(p.hz * 10) / 10, midi, cents, rms: Math.round(rms * 1000) / 1000 }
  if (midi !== prevMidi) {
    stable = 1
    if (midi !== lastOnsetMidi || silentFrames >= 2) armedAt = t
  } else stable++
  if (armedAt >= 0 && t - armedAt > 300) armedAt = -1 // never settled: a scrape, not a note
  if (armedAt >= 0 && stable >= SETTLE_FRAMES) {
    const at = Math.round(armedAt - hopMs)
    armedAt = -1
    if (at - lastOnsetAt > MIN_GAP_MS) {
      const onset: Onset = { t: at, midi, hz: state.now.hz, cents, rms: state.now.rms }
